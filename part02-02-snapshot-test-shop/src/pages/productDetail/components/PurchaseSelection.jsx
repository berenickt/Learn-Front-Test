...
const PurchaseSelection = ({ product }) => {
  ...
  const { user, isLogin } = useUserStore(state => pick(state, 'user', 'isLogin'))
  ...
  const addCart = () => {
    ...
    addCartItem(product, user.id, countRef.current)
    toast.success(`${product.title} ${countRef.current}개 장바구니 추가 완료!`, { id: TOAST_ID })
  }
  ...
  return (
    <Stack direction="column" spacing={2} mt={5}>
      ...
      />
      <Button variant="contained" startIcon={<CheckIcon />} onClick={purchaseProduct}>
        구매
      </Button>
    </Stack>
  )
