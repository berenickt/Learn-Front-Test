...
import { defaultDeleteFunc, defaultFetcher, defaultPostFunc } from '@/helpers/fetcher'

// https://github.com/horprogs/react-query/blob/master/src/utils/reactQuery.ts

export const useFetch = ({ url, options }) => {
  const context = useQuery([url, options?.params], ({ queryKey }) => defaultFetcher({ queryKey }), {
    enabled: !!url,
...
const useGenericMutation = ({ func, url, params, updater }) => {
  ...
  return useMutation(func, {
    onMutate: async data => {
      ...
      queryClient.setQueryData([url, params], oldData => (updater ? updater(oldData, data) : data))
