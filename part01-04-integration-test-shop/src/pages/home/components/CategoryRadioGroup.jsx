...
const CategoryRadioGroup = ({ categoryId, onChangeCategory }) => {
  ...
  return (
    <FormControl>
      ...
      <RadioGroup row name="category" onChange={onChangeCategory} value={categoryId}>
        <FormControlLabel value={ALL_CATEGORY_ID} control={<Radio />} id="All" label="All" />
        {data?.map(({ id, name }) => {
          return <FormControlLabel key={id} value={id} id={id} control={<Radio />} label={name} />
        })}
      </RadioGroup>
    </FormControl>
  )
