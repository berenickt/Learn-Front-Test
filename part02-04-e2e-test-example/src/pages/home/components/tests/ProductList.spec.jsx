...
import { mockUseUserStore, mockUseCartStore } from '@/utils/test/mockZustandStore'
...
it('로딩이 완료된 경우 상품 리스트가 제대로 모두 노출된다', async () => {
  ...
  productCards.forEach((el, index) => {
    ...
    expect(productCard.getByRole('button', { name: '장바구니' })).toBeInTheDocument()
    expect(productCard.getByRole('button', { name: '구매' })).toBeInTheDocument()
  })
})
...
describe('로그인 상태일 경우', () => {
  ...
  it('구매 버튼 클릭시 addCartItem 메서드가 호출되며, "/cart" 경로로 navigate 함수가 호출된다.', async () => {
    ...
    await user.click(screen.getAllByRole('button', { name: '구매' })[productIndex])

    expect(addCartItemFn).toHaveBeenNthCalledWith(1, data.products[productIndex], 10, 1)
  ...
  it('장바구니 버튼 클릭시 "장바구니 추가 완료!" toast를 노출하며, addCartItem 메서드가 호출된다.', async () => {
    ...
    await user.click(screen.getAllByRole('button', { name: '장바구니' })[productIndex])

    expect(addCartItemFn).toHaveBeenNthCalledWith(1, product, 10, 1)
    expect(screen.getByText(`${product.title} 장바구니 추가 완료!`)).toBeInTheDocument()
  })
})
